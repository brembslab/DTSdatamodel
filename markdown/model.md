...
The Drosophila Time Series (DTS) Data Model describes single time series experiments which are grouped into data sets. Data set files contain data items that are exclusively meta-data. The YAML data set files contain the grouping information linking each experiment to the data set, as well as a data set description and experimental comments. These files also contain information about which type of evaluation the data set was designed to be used with. Experiment files contain a meta-data header in XML and time series data in CSV.
...
## Data items in data set files

Data set files reside in the same directory as the data files and contain the experimental design of a data set by listing the different data files in the experimental groups. While data files are written by software, these data set files are written by humans. As YAML is easier to read for humans than XML, data files are in XML and data set files in YAML. The syntax loosely follows the Frictionless Data format at [https://specs.frictionlessdata.io/data-package](https://specs.frictionlessdata.io/data-package)

### Data item type: "experiment"

Each data set file starts with the **type **of the experiment (one of ‘torquemeter’ or ‘joystick’), a disambiguating **id **(SSRN, if available), a machine-readable and URL-usable **name** of the experiment, a free text **title** and a free text **description **of the experiment 
...
The **author **of the data set file is contained in the following fields:
...
three.groups	Collect parameters for a pair of two-group tests. Only defined for exactly three experimental groups, with exactly one group ‘resources/descriptions" containing "test” or “experimental”. In this case, the test/experimental group is tested against each of the other two groups.  The **title **is a short free-text field, **description **is a free-text description of the test(s) performed, **power **specifies if the post-hoc statistical power calculation should be done with the first group expected to be higher (‘greater’) or lower (‘less’) than the second group, or two-sided (‘two.sided’). The **data field **is a boolean  (0: FALSE, 1: TRUE) of whether the analyses should be performed.
...
Empty field. Presence or absence indicates that the data set is concluded or not and is used to create a permanent identifier when published.

### Data item type "epid"

Integer value that is created automatically by the synchronization procedure that publishes the data sets.
